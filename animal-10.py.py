# -*- coding: utf-8 -*-
"""
Created on Tue Nov 10 13:32:27 2020
...
###C:\Users\deepak\Desktop\Class_Project\archive\raw-img
path = "C:/Users/deepak/Desktop/Class_Project/random_data/rawdata"
...
for i, folder in enumerate(foldernames):
    ...
    for file in filenames:
        if count < 1400:
            ...
            files2.append(path + "/" + folder + "/" + file)
...
train, test = train_test_split(df, test_size=0.1507, random_state = 0)
...
history = model.fit_generator(train_flow, epochs = 4,validation_data = test_flow, callbacks=[ModelCheckpoint('VGG16.model', monitor='val_acc'), reduce_lr])
...
model1 = Sequential()
model1.add(Conv2D(6, kernel_size = 5, strides =1, padding = 'valid', activation = 'relu', input_shape = (32, 32, 3)))
model1.add(BatchNormalization())
model1.add(MaxPooling2D(pool_size = (2, 2), strides = 2))
model1.add(Dropout(0.1))

model1.add(Conv2D(16, kernel_size = 5, strides = 1, padding = 'valid', activation = 'relu'))
model1.add(BatchNormalization())
model1.add(MaxPooling2D(pool_size = (2, 2), strides = 2))
...
model2 = Sequential()
model2.add(Conv2D(96, kernel_size = 11, activation = 'relu',padding = 'valid', strides = 4, input_shape = (227, 227, 3)))
model2.add(MaxPooling2D(pool_size = (3, 3), strides = 2))

model2.add(Conv2D(256, kernel_size = 5, padding = 'same', strides = 1, activation = 'relu'))
model2.add(MaxPooling2D(pool_size = (3, 3), strides = 2))

model2.add(Conv2D(384, kernel_size = 3, padding = 'same', strides = 1,activation = 'relu'))
model2.add(MaxPooling2D(pool_size = (3, 3), strides = 1, padding = 'same'))

model2.add(Conv2D(256, kernel_size = 3, padding = 'same', strides = 1, activation = 'relu'))
model2.add(MaxPooling2D(pool_size = (3,3), strides = 2))

model2.add(Flatten())
model2.add(Dense(4096))
model2.add(Dense(1096))
...
##### Loss and Accuracy for validation and Training Dataset
loss_vals = history2.history['loss']
val_loss_vals = history2.history['val_loss']
